...
export interface Task extends request.CoreOptions, CheerioOptionsInterface {
...
export class Worker extends event.EventEmitter {
    ...
    private pushToQueue(t: Task) {
        ...
        self.pool.acquire(function (error, poolReference) {
            ...
            if (t.html) {
                self.onContent(null, t, {task:t, bodyRAW: t.html }, false);
    ...
    private buildHTTPReques(t: Task) {
        ...
        if (t.uri) {
            request(t.uri, t, function (error, msg, body) {
                ...
                self.onContent(error, t, {task:t,  incomingMessage: msg, bodyRAW: body });
            });
        }
    ...
    private onContent(error, t: Task, result: Result, fromCache: boolean = false) {
        ...
        if (isHTML && t.method !== 'HEAD' && result.incomingMessage) {
            if (t.cheerioConvert) {
                result.$ = cheerio.load(result.body, t);
            }
            t.callback(null, result.incomingMessage, result);
        } else {
            t.callback(null, <http.IncomingMessage>{}, null);
        }
        return self.emit('pool:release', t);
    }
